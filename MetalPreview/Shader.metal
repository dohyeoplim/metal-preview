...
half4 gridInterpolation(float2 coords, device const half4 *colors, float4 gridRange, int2 gridSize, int lastIndex, float time) {
    float a = (sin(time * 1.2) + sin(time * 1.9)) * 0.25 + 0.5;
    float b = (sin(time * 1.5) + sin(time * 2.3)) * 0.25 + 0.5;
    float c = (sin(time * 1.8) + sin(time * 2.7)) * 0.25 + 0.5;
    float d = (sin(time * 2.2) + sin(time * 3.1)) * 0.25 + 0.5;
    ...
    coords.x = hermite(0.0, 1.0, 5.0 * x0, 5.0 * x1, coords.x);
    coords.y = hermite(0.0, 1.0, 5.0 * y0, 5.0 * y1, coords.y);
...
[[ stitchable ]]
half4 gradient(float2 position, float4 bounds, float size, float time, device const half4 *colors, int count) {
    ...
    half4 result = gridInterpolation(coords, colors, float4(gridRange), gridSize, gridLastIndex, time * 0.3);
